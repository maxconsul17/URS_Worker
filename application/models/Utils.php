...
class Utils extends CI_Model {
	 ...
	 * @return array
	 */
	function getEmplist($deptid = "", $division='', $caption='', $tnt='',$employmentstat='',$nameformat='',$includeResigned=false,$dateGen='', $isShowDeptAndTnt = false,$employeeid='', $isactive="", $campus='', $sDept='', $sAlp='', $company='', $office='', $soffc="", $multiple=false, $batch_access = false){
		...
		if($employeeid && $employeeid != "null" && $employeeid != "all"){
			if($multiple){
		...
		if($company) 		array_push($cond,"company_campus = ".$this->db->escape($company)."");
		if($isactive){
			if ($isactive == '1') {
		...
		if ($batch_access === true) {
			$batchaccess = $this->extras->getBatchAccess();
			if ($batchaccess && !in_array("all", explode(",", $batchaccess))) {
				$wC .= " AND employeeid IN (SELECT employeeid FROM payroll_batch_emp WHERE FIND_IN_SET (base_id,'$batchaccess') ) ";
			}
		}
		...
		foreach($query as $val){
			$query = $this->db->query("SELECT * FROM payroll_employee_salary WHERE employeeid = '".$val->employeeid."'");
			// if($query->num_rows() > 0){
    ...
    function getEmployeeIDList($deptid="",$division='',$tnt='',$employeeid='',$orderby='', $campus = '', $department = '', $dept_select="",$office="",$isactive="",$company_campus=""){
		...
		if($tnt){
			...
			else $wC .= " AND a.teachingtype='nonteaching' AND a.trelated = '1'";
		}
		// if($tnt)       		$wC .= " AND a.teachingtype='$tnt'";
		if($employeeid && $employeeid != 'all')  	$wC .= " AND a.employeeid='$employeeid'";
		if($campus && strtolower($campus) != 'all') $wC .= " AND a.campusid='$campus'";
	...
	function getNotif($tbl_emplist=''){
		...
		switch ($tbl_emplist) {
			case 'substitute_app_emplist':
				...
				break;
			case 'change_sched_app_emplist':
				$res = $this->db->query("SELECT Count(DISTINCT csae.`base_id`) as count FROM `change_sched_app_emplist` as csae LEFT JOIN change_sched_app as csa ON csa.`id`=csae.`base_id` WHERE csae.employeeid='$employeeid' AND csa.isread='0'");
				break;
			default:
				# code...
				break;
	...
	function getNotifmySCRequest(){
		$username = $this->session->userdata("username");
		return $this->db->query("SELECT * FROM service_credit_app a INNER JOIN sc_app_emplist_new b ON a.id = b.base_id WHERE employeeid = '$username' AND a.isread = '0' GROUP BY a.id")->num_rows();
	}

	function getNotifmySCUseRequest(){
		$username = $this->session->userdata("username");
		return $this->db->query("SELECT * FROM sc_app_use a INNER JOIN sc_app_use_emplist_new b ON a.id = b.base_id WHERE employeeid = '$username' AND a.isread = '0' GROUP BY a.id")->num_rows();
	}
	...
	function getNotifLeaveRequestByAdmin(){
		$username = $this->session->userdata("username");
		$query = $this->db->query("
			SELECT lab.id FROM leave_app_base lab 
			INNER JOIN leave_app_emplist lae ON lae.`base_id`=lab.`id` 
			INNER JOIN user_info ui ON lab.`applied_by`=ui.`username` 
			WHERE lab.`isread`=0 AND lae.`employeeid`='$username' GROUP BY lab.`id`
		");
	...
	function getNotifManageLEAVE(){
		$username = $this->session->userdata("username");
		return $this->db->query("SELECT b.* FROM leave_app_emplist a INNER JOIN leave_app_base b ON b.id = a.base_id WHERE FIND_IN_SET('$username', approver_id) > 0 AND a.status = 'PENDING' AND b.status = 'PENDING' AND ongoing_approver = 'me'")->num_rows();
	}
	...
	function getNotifManageOB(){
		$username = $this->session->userdata("username");
		// return $this->db->query("SELECT * FROM ob_app_emplist WHERE FIND_IN_SET('$username', approver_id) > 0 AND status = 'PENDING' AND ongoing_approver = 'me'")->num_rows();
		return $this->db->query("SELECT * FROM ob_app WHERE id IN(SELECT base_id FROM ob_app_emplist WHERE FIND_IN_SET('{$username}', approver_id) > 0 AND STATUS = 'PENDING' AND ongoing_approver = 'me') AND ob_type = 'ob' AND `status` = 'PENDING'")->num_rows();
	}
	...
	function getNotifManageMonetize(){
		$username = $this->session->userdata("username");
		return $this->db->query("SELECT * FROM monetize_app a INNER JOIN monetize_app_emplist b ON a.id = b.base_id WHERE FIND_IN_SET('$username', b.approver_id) > 0 AND b.status = 'PENDING' AND b.ongoing_approver = 'me' AND a.app_status = 'PENDING'")->num_rows();
	}

	function getNotifManageSC(){
		$username = $this->session->userdata("username");
		return $this->db->query("SELECT * FROM service_credit_app a INNER JOIN sc_app_emplist_new b ON a.id = b.base_id WHERE FIND_IN_SET('$username', b.approver_id) > 0 AND b.status = 'PENDING' AND b.ongoing_approver = 'me' AND a.app_status = 'PENDING' AND a.isread = '0'")->num_rows();
	}

	function getNotifManageSCUse(){
		$username = $this->session->userdata("username");
		return $this->db->query("SELECT * FROM sc_app_use a INNER JOIN sc_app_use_emplist_new b ON a.id = b.base_id WHERE FIND_IN_SET('$username', b.approver_id) > 0 AND b.status = 'PENDING' AND b.ongoing_approver = 'me' AND a.app_status = 'PENDING'")->num_rows();
	}

	function getNotifOvertimeRequest(){
		$username = $this->session->userdata("username");
		$q1 = $this->db->query("SELECT * FROM ot_app a INNER JOIN ot_app_emplist b ON a.id = b.base_id WHERE employeeid = '$username' AND a.isread = '0' GROUP BY a.id")->num_rows();

		$q2 = $this->db->query("SELECT * FROM employee_cto_usage a INNER JOIN employee_cto_usage_emplist b ON a.id = b.base_id WHERE applied_by = '$username' AND a.isread = '0' GROUP BY a.id")->num_rows();
		// echo "<pre>"; print_r($this->db->last_query()); die;

		return ($q1 + $q2);
	}

	function getNotifMonetizeRequest(){
		$username = $this->session->userdata("username");
		return $this->db->query("SELECT * FROM monetize_app a INNER JOIN monetize_app_emplist b ON a.id = b.base_id WHERE employeeid = '$username' AND a.isread = '0' GROUP BY a.id")->num_rows();
	}

	function getNotifManageOvertime(){
		$username = $this->session->userdata("username");
		$q1 = $this->db->query("SELECT * FROM ot_app_emplist a INNER JOIN ot_app b ON b.id = a.`base_id` WHERE FIND_IN_SET('$username', a.approver_id) > 0 AND a.status = 'PENDING' AND a.ongoing_approver = 'me' AND b.status = 'PENDING'")->num_rows();
		$q2 = $this->db->query("SELECT * FROM employee_cto_usage_emplist a INNER JOIN employee_cto_usage b ON a.base_id = b.id WHERE FIND_IN_SET('$username', a.approver_id) > 0 AND a.status = 'PENDING' AND a.ongoing_approver = 'me' AND b.app_status = 'PENDING' GROUP BY a.base_id")->num_rows();
		return ($q1 + $q2);
	}

	function getNotifManageChangeSched(){
		$username = $this->session->userdata("username");
		$q1 = $this->db->query("SELECT * FROM change_sched_app_emplist WHERE FIND_IN_SET('$username', approver_id) > 0 AND status = 'PENDING' AND ongoing_approver = 'me'")->num_rows();
		return $q1;
	}
	...
	function getClassificationList($caption=''){
        $returns = array();
        if (isset($caption)) {
            $returns = array(""=>$caption);
        }
        ...
        $this->db->order_by("description","asc");
        $q = $this->db->get("faculty_load_classification"); 
        for($t=0;$t<$q->num_rows();$t++){
          $row = $q->row($t);
          $returns[$row->id] = $row->description;
        }
        return $returns;
    }
    ...
    function getAIMSDepartment($caption=''){
        ...
        $this->db->select("education_level,description");
        $this->db->where("education_level !=", "");
        $this->db->group_by("education_level");
        $this->db->order_by("description","asc");
        $q = $this->db->get("aims_department"); 
        for($t=0;$t<$q->num_rows();$t++){
          $row = $q->row($t);
          $returns[Globals::_e($row->education_level)] = Globals::_e($row->description);
        }
        return $returns;
    }
	...
	function getEmployeeOffice($empid){
		...
		if($empDepartment->num_rows() > 0) $empDepartment->row()->office;
		else return false;
	}

	function getEmployeeDateActive($empid){
		$empDateActive = $this->db->query("SELECT date_active FROM employee WHERE employeeid = '$empid' ");
		if($empDateActive->num_rows() > 0) $empDateActive->row()->date_active;
		else return false;
	}

	function getScheduleEffectiveDate($empid){
		$empDateActive = $this->db->query("SELECT * FROM employee_schedule WHERE employeeid = '$empid' ");
		if($empDateActive->num_rows() > 0){
			$dateactive = date("Y-m-d", strtotime($empDateActive->row()->dateedit));
			return $dateactive;
		}
		else{ 
			return false;
	...
	function getPayrollSchedule($employeeid){
		$query = $this->db->query("SELECT schedule FROM payroll_employee_salary WHERE  employeeid = '$employeeid'");
		if($query->num_rows() > 0) return $query->row()->schedule;
		return "";
	}

	function cutoffCount($dfrom, $dto){
		return $this->db->query("SELECT * FROM payroll_cutoff_config WHERE startdate BETWEEN '$dfrom' AND '$dto' OR enddate BETWEEN '$dfrom' AND '$dto' GROUP BY CutoffID")->num_rows();
	}
