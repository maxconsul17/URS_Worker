...
{   
    ...
    public function processCalculationUpdate() {
		$att_list = $this->worker_model->getEmployeeAttToUpdate(); 
		if ($att_list) {
			try {
				
				$employeeid = $att_list["employeeid"];
				$dfrom = $att_list["date"];
				$dto = $att_list["date"];
				$this->worker_model->update_attendance_status($dfrom, $employeeid);
				// echo "<pre>";print_r($att_list);die;

				$this->calculate_attendance($employeeid, $dfrom, $dto);
			} catch (Exception $e) {
			}
		}
	}
    ...
    {
       return $this->worker_model->getHasUpdateJob();
    }
